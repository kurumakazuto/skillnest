...
const idError = computed(() => {
  ...
  if (isUppercaseIncluded(id))
    return "メールアドレスは小文字で入力してください";
...
const passwordError = computed(() => {
  ...
  if (isMissingLetterOrNumber(password))
...
<template>
  <div class="w-96 text-left space-y-5 mb-12">
    ...
    />
    <span class="w-full text-red-500" v-if="idError">{{
      `！ ${idError}`
    }}</span>
    ...
    />
    <span class="w-full text-red-500" v-if="passwordError">{{
      `！ ${passwordError}`
    }}</span>
  </div>
  <div class="text-center">
    <UiButton
      label="登録"
      :disabled="Boolean(idError) || Boolean(passwordError)"
    />
  </div>
</template>
